...
public class NetworkGraphBuilder implements GraphBuilder {
    ...
    private abstract static class AbstractGraphBuilder extends DefaultTopologyVisitor {
        ...
        protected abstract void add3wtFeeder(Fictitious3WTNode starNode, Feeder3WTNode firstOtherLegNode,
                                             Feeder3WTNode secondOtherLegNode, Terminal terminal);
        ...
        private FeederNode createFeederLineNode(Graph graph, Line line, Branch.Side side) {
            ...
            return new FeederLineNode(id, name, equipmentId, false, graph, s, new VoltageLevelInfos(vlOtherSide.getId(), vlOtherSide.getName(), vlOtherSide.getNominalV()));
        }
        ...
        private FeederNode createFeederNode(Graph graph, Injection injection) {
            ...
            switch (injection.getType()) {
                case GENERATOR:
                    return FeederInjectionNode.createGenerator(injection.getId(), injection.getName(), graph);
                case LOAD:
                    return FeederInjectionNode.createLoad(injection.getId(), injection.getName(), graph);
                case HVDC_CONVERTER_STATION:
                    return FeederInjectionNode.createVscConverterStation(injection.getId(), injection.getName(), graph);
                case STATIC_VAR_COMPENSATOR:
                    return FeederInjectionNode.createStaticVarCompensator(injection.getId(), injection.getName(), graph);
                case SHUNT_COMPENSATOR:
                    return ((ShuntCompensator) injection).getbPerSection() >= 0 ? FeederInjectionNode.createCapacitor(injection.getId(), injection.getName(), graph)
                                                                                : FeederInjectionNode.createInductor(injection.getId(), injection.getName(), graph);
                case DANGLING_LINE:
                    return FeederInjectionNode.createDanglingLine(injection.getId(), injection.getName(), graph);
                default:
        ...
        public void visitThreeWindingsTransformer(ThreeWindingsTransformer transformer,
            ...
            if (graph.isForVoltageLevelDiagram()) {
                ...
                Feeder3WTNode firstOtherLegNode = Feeder3WTNode.create(firstOtherLegNodeId, transformer.getName(), transformer.getId(), graph, firstOtherLegSide);

                // create second other leg feeder node
                String secondOtherLegNodeId = transformer.getId() + "_" + secondOtherLegSide.name();
                Feeder3WTNode secondOtherLegNode = Feeder3WTNode.create(secondOtherLegNodeId, transformer.getName(), transformer.getId(), graph, secondOtherLegSide);
                ...
                Feeder3WTNode legNode = Feeder3WTNode.create(id, transformer.getName(), transformer.getId(), graph, FeederWithSideNode.Side.valueOf(side.name()));
    ...
    private class NodeBreakerGraphBuilder extends AbstractGraphBuilder {
        ...
        @Override
        protected void add3wtFeeder(Fictitious3WTNode starNode, Feeder3WTNode firstOtherLegNode, Feeder3WTNode secondOtherLegNode, Terminal terminal) {
            ConnectablePosition.Feeder feeder = getFeeder(terminal);
            if (feeder != null) {
