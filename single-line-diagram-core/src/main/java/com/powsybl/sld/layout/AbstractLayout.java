...
public abstract class AbstractLayout implements Layout {
    ...
    protected void manageSnakeLines(BaseGraph graph, LayoutParameters layoutParameters) {
        for (MiddleTwtNode multiNode : graph.getMultiTermNodes()) {
            ...
            if (adjacentNodes.size() == 2) {
                List<Point> pol = calculatePolylineSnakeLine(layoutParameters, adjacentNodes.get(0),
                        adjacentNodes.get(1), true);
                ...
                List<Point> pol1 = calculatePolylineSnakeLine(layoutParameters, adjacentNodes.get(0),
                        adjacentNodes.get(1), true);
                List<Point> pol2 = calculatePolylineSnakeLine(layoutParameters, adjacentNodes.get(1),
                        adjacentNodes.get(2), false);
        ...
        for (BranchEdge lineEdge : graph.getLineEdges()) {
            ...
            lineEdge.setSnakeLine(
                    calculatePolylineSnakeLine(layoutParameters, adjacentNodes.get(0), adjacentNodes.get(1), true));
        }
    }
    ...
    protected static BusCell.Direction getNodeDirection(Node node, int nb) {
        ...
        BusCell.Direction dNode = node.getCell() != null ? ((ExternCell) node.getCell()).getDirection()
                : BusCell.Direction.TOP;
     ...
     * This is a default implementation of 'calculatePolylineSnakeLine' for a
     * horizontal layout
     */
    protected static List<Point> calculatePolylineSnakeLineForHorizontalLayout(LayoutParameters layoutParam, Node node1,
            Node node2,
    ...
    private static void addMiddlePoints(LayoutParameters layoutParam, Node node1, Node node2,
        ...
        if (dNode1 == dNode2) {
            ...
            VoltageLevelGraph rightestVoltageLevel = node1.getVoltageLevelGraph().getX() > node2.getVoltageLevelGraph()
                    .getX() ? node1.getVoltageLevelGraph() : node2.getVoltageLevelGraph();
            ...
            double xBetweenGraph = xMaxGraph - vlPadding.getLeft()
                    - (infosNbSnakeLines.getNbSnakeLinesVerticalBetween().compute(idMaxGraph, (k, v) -> v + 1) - 1)
                            * layoutParam.getHorizontalSnakeLinePadding();
    ...
    private static double getVerticalShift(LayoutParameters layoutParam, BusCell.Direction dNode1,
            Map<BusCell.Direction, Integer> nbSnakeLinesTopBottom) {
        if (dNode1 == BusCell.Direction.BOTTOM) {
            return Math.max(nbSnakeLinesTopBottom.get(dNode1) - 1, 0) * layoutParam.getVerticalSnakeLinePadding()
                    + layoutParam.getVoltageLevelPadding().getBottom();
        } else {
            return -Math.max(nbSnakeLinesTopBottom.get(dNode1) - 1, 0) * layoutParam.getVerticalSnakeLinePadding()
                    - layoutParam.getVoltageLevelPadding().getTop();
        }
    }
    ...
    protected List<List<Point>> splitPolyline2(List<Point> points, Node multiNode) {
        ...
        // we need to reverse the order for the second part as the edges are always from
        // middleTwtNode to twtLegNode
    ...
    protected List<List<Point>> splitPolyline3(List<Point> points1, List<Point> points2, Node coord) {
        // for the first new edge, we keep all the original first polyline points,
        // except the last one
        List<Point> part1 = new ArrayList<>(points1.subList(0, points1.size() - 1));

        // for the second new edge, we keep the last two points of the original first
        // polyline (in reverse order
        // as the edges are always from middleTwtNode to twtLegNode
        // we need to create a new point to avoid having a point shared between part1
        // and part2
        List<Point> part2 = Arrays.asList(points1.get(points1.size() - 1), new Point(points1.get(points1.size() - 2)));

        // the third new edge is made with the original second polyline, except the
        // first point (in reverse order
        // as the edges are always from middleTwtNode to twtLegNode)
        LinkedList<Point> part3 = new LinkedList<>();
        points2.stream().skip(1).forEach(part3::addFirst);

        // the fictitious node point is the second to last point of the original first
        // polyline (or the second of the original second polyline)
        coord.setCoordinates(points2.get(1));

        return Arrays.asList(part1, part2, part3);
    }

    protected static double getWidthVerticalSnakeLines(String vlGraphId, LayoutParameters layoutParameters,
            InfosNbSnakeLinesHorizontal infosNbSnakeLines) {
        return Math.max(infosNbSnakeLines.getNbSnakeLinesVerticalBetween().get(vlGraphId) - 1, 0)
                * layoutParameters.getHorizontalSnakeLinePadding();
    }

    protected static double getHeightSnakeLines(LayoutParameters layoutParameters, BusCell.Direction top,
            InfosNbSnakeLinesHorizontal infosNbSnakeLines) {
        return Math.max(infosNbSnakeLines.getNbSnakeLinesTopBottom().get(top) - 1, 0)
                * layoutParameters.getVerticalSnakeLinePadding();
    }
}
