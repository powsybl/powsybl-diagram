...
public class GravityForceByEdgeNumber<V, E> implements Force<V, E> {
    ...
    @Override
    public Vector2D calculateForce(V forThisVertex, Point correspondingPoint, ForceGraph<V, E> forceGraph, Double forceIntensity) {
        ...
        double magnitude = forceIntensity * (forceGraph.getGraph().degreeOf(forThisVertex) + 1);
        return Vector2D.calculateUnitVector(correspondingPoint, ORIGIN).multiply(magnitude);
    }

    @Override
    public Vector2D calculateForce(V forThisVertex, Point correspondingPoint, ForceGraph<V, E> forceGraph, Double forceIntensity, OptimizationArtifactsContainer optimizationArtifacts) {
        ...
        return calculateForce(forThisVertex, correspondingPoint, forceGraph, forceIntensity);
    }
}
