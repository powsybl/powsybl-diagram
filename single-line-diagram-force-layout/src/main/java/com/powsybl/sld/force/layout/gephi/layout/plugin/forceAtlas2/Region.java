...
public class Region {
    ...
    public void applyForce(Node n, ForceFactory.AbstractRepulsionForce repulsionForce, double theta) {
        if (nodes.size() < 2) {
            Node regionNode = nodes.get(0);
            repulsionForce.apply(n, regionNode);
        } else {
            double distance = Math.sqrt((n.x() - massCenterX) * (n.x() - massCenterX) + (n.y() - massCenterY) * (n.y() - massCenterY));
            if (distance * theta > size) {
                repulsionForce.apply(n, this);
            } else {
                for (Region subregion : subregions) {
