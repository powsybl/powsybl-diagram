...
class SubnetworkHighlightTest extends AbstractTest {
    ...
    public static Network createwith3wt(NetworkFactory networkFactory) {
        ...
        addLines(network0, vlhv1, vlhv2, vlhv3, nhv1, nhv2, bus3);
        addTransformers(p1, p2, vlgen, vlhv1, vlhv2, vlload, ngen, nhv1, nhv2, nload);
        addInjections(vlload, vlgen, nload, ngen);
        addThreeWindingsTransformer(p3, vlhv3, vlhv4, vlhv5, bus3, bus4, bus5);
        addHvdc(network0, vlhv3, vlload, nload);
        return network0;
    }

    private static void addLines(Network network0,
                                 VoltageLevel vlhv1, VoltageLevel vlhv2, VoltageLevel vlhv3,
                                 Bus nhv1, Bus nhv2, Bus bus3) {
        network0.newLine()
            .setId(EurostagTutorialExample1Factory.NHV1_NHV2_1)
    ...
    private static void addTransformers(Substation p1, Substation p2,
                                        VoltageLevel vlgen, VoltageLevel vlhv1, VoltageLevel vlhv2, VoltageLevel vlload,
                                        Bus ngen, Bus nhv1, Bus nhv2, Bus nload) {
    ...
    private static void addInjections(VoltageLevel vlload, VoltageLevel vlgen, Bus nload, Bus ngen) {
        vlload.newLoad()
            .setId("LOAD")
            .setBus(nload.getId())
    ...
    private static void addThreeWindingsTransformer(Substation p3, VoltageLevel vlhv3, VoltageLevel vlhv4, VoltageLevel vlhv5, Bus bus3, Bus bus4, Bus bus5) {
    ...
    private static void addHvdc(Network network0, VoltageLevel vlhv3, VoltageLevel vlload, Bus nload) {
