...
public abstract class AbstractStyleProvider implements StyleProvider {
    ...
    public List<String> getFeederInfoStyles(FeederInfo info) {
        ...
        if (info instanceof EdgeFeederInfo && ((EdgeFeederInfo) info).isDirectional()) {
            styles.add(((EdgeFeederInfo) info).getDirection() == LabelProvider.LabelDirection.OUT ? StyleClassConstants.OUT_CLASS : StyleClassConstants.IN_CLASS);
        }
        return styles;
    }
}
